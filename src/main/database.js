...
function getOpenDbInstance() {
    if (!dbInstance || !dbInstance.open) {
        try {
            dbInstance = new sqlite3.Database(dbPath, (err) => {
                if (err) {
                    console.error('Error al abrir/crear la base de datos Presto Argento:', err.message);
                    // dbInstance podría no estar completamente inicializado o ser null aquí.
                    // Si el error es en la conexión, dbInstance.open podría ser falso.
                } else {
                    console.log(`Conectado a la base de datos SQLite Presto Argento en: ${dbPath}`);
                    dbInstance.exec('PRAGMA foreign_keys = ON;', (pragmaErr) => {
                        if (pragmaErr) {
            ...
            // Importante: La conexión es asíncrona. sqlite3.Database no devuelve una promesa.
            // Para operaciones inmediatas, esto podría ser un problema si la instancia no está lista.
            // Sin embargo, las operaciones de sqlite3 (get, all, run, serialize) se encolan y esperan a que la conexión esté lista.
        } catch (error) {
            console.error('Excepción al crear instancia de base de datos:', error.message);
            dbInstance = null; // Asegurarse de que esté null si falla la creación
        }
    }
    // Si dbInstance es null después del intento de conexión (por error síncrono o asíncrono inmediato),
    // las funciones get/all/run fallarán, lo cual es el comportamiento esperado.
    return dbInstance; 
}
...
function closeDatabaseConnection() {
    return new Promise((resolve, reject) => {
        if (dbInstance && dbInstance.open) {
            ...
            console.log('La conexión de la base de datos ya está cerrada o no inicializada.');
            dbInstance = null; // Asegurar que esté null
            resolve();
        }
    });
}
...
const initDatabase = () => {
    return new Promise((resolve, reject) => {
        const db = getOpenDbInstance();
        if (!db) { // Chequeo síncrono por si la creación falló inmediatamente
            return reject(new Error("No se pudo obtener una instancia de base de datos para inicializar."));
        }
        // Esperar un breve momento para asegurar que la conexión asíncrona tenga oportunidad de establecerse
        // Esto es un workaround. Idealmente, getOpenDbInstance debería devolver una promesa.
        setTimeout(() => {
            if (!db.open) { // Chequear si la conexión realmente se abrió
                 return reject(new Error("La instancia de base de datos no está abierta después del intento de conexión."));
            }
            db.serialize(() => {
                // Tabla de Usuarios
                db.run(`
            ...
            // Tabla de Información de la Empresa
                db.run(`
                    ...
                    default_daily_arrears_rate REAL DEFAULT 0.001, -- Nueva columna
                    admin_username TEXT, 
                    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
                )
            `, (err) => { 
                ...
                // Insertar datos por defecto si la tabla está vacía
                    db.get("SELECT COUNT(*) as count FROM company_info WHERE id = 1", (err, row) => {
                        ...
                        if (row.count === 0) {
                            // El DEFAULT 0.001 en la schema se encargará de la nueva columna
...
const get = (sql, params = []) => {
    return new Promise((resolve, reject) => {
        const db = getOpenDbInstance();
        if (!db) return reject(new Error("Database connection not available for get."));
...
const all = (sql, params = []) => {
    return new Promise((resolve, reject) => {
        const db = getOpenDbInstance();
        if (!db) return reject(new Error("Database connection not available for all."));
...
const run = (sql, params = []) => {
    return new Promise((resolve, reject) => {
        const db = getOpenDbInstance();
        if (!db) return reject(new Error("Database connection not available for run."));
...
module.exports = {
    // db: getOpenDbInstance(), // Podría exportarse así si se quiere mantener la sintaxis db.get, etc.
    // o hacer que getOpenDbInstance devuelva una promesa y se maneje en cada llamada.
    // Por ahora, las funciones get/all/run internamente obtienen la instancia.
    initDatabase,
    closeDatabaseConnection, // Nueva función exportada
    get,
    all,
    run
};